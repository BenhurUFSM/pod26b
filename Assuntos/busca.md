...
Uma tarefa frequente quando se tem um conjunto de registros é se encontrar um desses registros em particular.
De forma geral, um registro contém uma chave de busca e o restante dos dados. Uma operação de busca consiste em encontrar os dados associados à determinada chave.
Falaremos primeiro na busca por dados que estão em memória interna, depois na busca por dados localizados em arquivos.

Quando em memória, os dados podem estar em um vetor ou em uma lista encadeada ou em uma árvore de busca ou em alguma outra estrutura de dados.
Veremos algumas dessas estruturas neste capítulo.

### Busca sequencial

Quando a estrutura que armazena os registros só permite o acesso sequendial (como uma lista encadeada) ou quando a ordem dos registros na estrutura não é relacionada à chave de busca, não se tem muito recurso para otimizar a busca por um dado, e se usa força bruta, verificando cada dado na estrutura para ver se contém a chave buscada.

Se não tem chave repetida, a busca pode parar quando encontrar a chave buscada. Se se tem chave repetida, ou se a chave não for encontrada, a busca tem que ser exaustiva, em todos os dados.

Se os dados estiverem ordenados por ordem da chave, a busca pode parar tão logo se encontre um dado que tem uma chave superior à buscada.

Se os dados estiverem ordenados, e a estrutura tiver acesso direto por posição (um vetor), pode-se utilizar a busca binária:
1. compara-se a chave buscada com a que está no meio da região de busca
2. se a chave buscada for maior, a região de busca passa a conter somente os dados que estão após o local buscado
3. se a chave for menor, a região de busca passa a ser os dados que estão antes.
4. repita até encontrar a chave buscada ou a região de busca ficar vazia.

Em pseudo código:
```c
dado_t busca(chave, dado_t v[], int inicio, int fim)
{
  if (inicio > fim) return NAO_EXISTE;
  int i = (inicio + fim) / 2;
  switch (compara(chave, v[i].chave)) {
    case IGUAIS: return v[i].dado;
    case ANTES:  return busca(chave, v, inicio, i - 1);
    case DEPOIS: return busca(chave, v, i + 1, fim);
  }
}
```
Em versão não recursiva:
```c
dado_t busca(chave, dado_t v[], int inicio, int fim)
{
  while (inicio < fim) {
    ...
    switch (compara(chave, v[i].chave)) {
      case IGUAIS: return v[i].dado;
      case ANTES:  fim = i - 1; break;
      case DEPOIS: inicio = i + 1; break;
    }
  }
  return NAO_EXISTE;
}
```
Cada comparação elimina metade do espaço de busca, a complexidade da busca binária é $O(\log n)$.
O custo para manter o vetor, entretanto, é alto, $O(n)$ tanto na inserção quanto na remoção de um dado, porque os dados têm que ser movidos fisicamente na memória.
Em uma estrutura mais dinâmica, esse custo pode ser reduzido.

### Árvore binária de busca (ABB)

Organiza-se a árvore binária de forma que os dados contidos na subárvore esquerda contêm chaves menores que o dado contido na raiz, e os da direita chaves maiores (em todos os níveis da árvore).
Dessa forma, ao se comparar a chave buscada com a chave do dado na raiz da árvore elimina-se uma subárvore inteira da busca.
Em código:
```c
dado_t busca(ABB *a, chave)
{
  if (vazia(a)) return NAO_EXISTE;
  switch(compara(chave, a->chave)) {
    case IGUAIS: return a->dado;
    case ANTES:  return busca(a->esq, chave);
    case DEPOIS: return busca(a->dir, chave);
  }
}
```
É semelhante à busca binária, e pode ter complexidade $O(\log n)$, se a árvore tiver uma altura próxima à menor altura possível para uma árvore de $n$ nós. Infelizmente, isso não é garantido pelos algoritmos simples de inserção e remoção da árvore, que para manter a ordem adequada dos nós da árvore (para que seja de busca), não têm muita liberdade sobre onde realizar a inserção ou remoção.

No caso da inserção, deve-se inserir uma nova folha na árvore, o e local de inserção é encontrado da mesma forma que a busca:
```c
ABB *insere(ABB *a, dado)
{
  if (vazia(a)) return abb_cria(dado);
  switch(compara(dado.chave, a->chave)) {
    case IGUAIS:
...
```
A remoção é um pouco mais trabalhosa, porque tem 3 casos:
- o nó a remover é folha — basta remover
- o nó a remover tem um filho — substitui o nó pelo filho
- o nó a remover tem dois filhos — substitui o valor do nó pelo maior valor na subárvore esquerda ou pelo menor da subárvore direita e remove esse valor da árvore.
```c
ABB *remove(ABB *a, chave)
{
  if (vazia(a)) return a; // não existe
  switch(compara(chave, a->chave)) {
    case IGUAIS:
      if (vazia(a->esq) && vazia(a->dir)) {
        ...
        dado = maior_dado(a->esq);
        altera_dado_no(a, dado);
        a->esq = remove(a->esq, dado.chave);
      }
      break;
    case ANTES:
...
```
A complexidade da inserção e remoção também dependem da altura da árvore.
Para se ter uma complexidade próxima ao melhor possível ($O(\log n)$), deve-se manter a árvore *equilibrada* (com uma altura não muito maior que o menor possível).
Existem várias formas de se manter uma árvore binária de busca equilibrada a um custo não muito alto.

#### Árvore AVL

Uma árvore AVL é uma árvore de busca binária auto-equilibrada.
Ela define um valor chamado de *fator de equilíbrio*, que é a diferença entre a altura do filho esquerdo e a do filho direito.
Uma AVL:
- é uma ABB;
- tem fator de equilíbrio 0 ou 1 ou -1;
- tem filhos que são AVL.

Para manter uma árvore AVL, deve-se, em toda alteração da árvore (inserção e remoção), verificar se ela deixou de ser AVL e reequilibrá-la se for o caso.
Isso pode ser feito nas funções acima incluindo uma chamada a uma função de verificação e correção logo antes do retorno final.
Como as funções de inserção e remoção são recursivas, essa verificação será feita em todos os nós potencialmente afetados pela inserção ou remoção (que são os que podem necessitar correção).
...
![rotação AVL](https://github.com/BenhurUFSM/ed21a/blob/main/Complementos/ed21a-rota%C3%A7%C3%A3o%20AVL.png)
...
O caso de um desequilíbrio do nó X devido a uma inserção em seu lado direito (um valor maior que X) poderia ser tratado de forma semelhante, trocando esquerda por direita, como se a árvore fosse espelhada.
...
A função para reequilibrar a árvore poderia ser como abaixo (a ser chamada no final da inserção e remoção, trocando o `return a;` por `return avl_equilibra(a);`):
 
 ```c
    AVL *avl_equilibra(AVL *a)
    {
       if (vazia(a)) return a;  // pode acontecer na remoção
       calcula_altura(a); // pode ter mudado pela alteração na árvore
       int fat_eq = fator_de_equilíbrio(a);
       ...
       if (fat_eq == 2) { // filho mais alto está na esquerda
          if (fator_de_equilíbrio(a->esq) == -1) {
             return rotação_dupla_à_esquerda(a);
          } else {
             return rotação_simples_à_esquerda(a);
          }
 ...
 ```c
    ...
    int fator_de_equilíbrio(AVL *a)
    {
       return altura(a->esq) - altura(a->dir);
    }
    
    AVL *rotação_simples_à_esquerda(AVL *a)
    {
    ...
    AVL *rotação_dupla_à_esquerda(AVL *a)
    {
       a->esq = rotação_simples_à_direita(a->esq);
       return rotação_simples_à_esquerda(a);
    }
    
    AVL *rotação_simples_à_direita(AVL *a)
    {
          // mesma coisa pro outro lado
    }
    
    AVL *rotação_dupla_à_direita(AVL *a)
    {
          // mesma coisa pro outro lado
    }
 ```
